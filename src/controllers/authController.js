...
const client = require("../utils/redies")
...
exports.getAllRegisterUsers = async (req, res) => {
  try {
    const cacheKey = "getAllRegisterUsers";

    try {
      const cachedData = await client.get(cacheKey);
      if (cachedData) {
        console.log("CACHE HIT ✅ - getAllRegisterUsers");
        return res.status(200).json(JSON.parse(cachedData));
      } else {
        console.log("CACHE MISS ❌ - getAllRegisterUsers");
      }
    } catch (err) {
    ...
    try {
      await client.set(cacheKey, JSON.stringify(users), { EX: 1800 });
    } catch (err) {
...
exports.getRegisterUserById = async (req, res) => {
  try {
    ...
    const cacheKey = `getRegisterUserById:${userId}`;

    // 🔹 Check Redis First
    try {
      const cachedData = await client.get(cacheKey);
      if (cachedData) {
        console.log("CACHE HIT ✅ - getRegisterUserById");
        return res.status(200).json(JSON.parse(cachedData));
      } else {
        console.log("CACHE MISS ❌ - getRegisterUserById");
      }
    } catch (err) {
      console.log("Redis read error:", err);
    }
    ...
    const user = await User.findById(userId)
      .select("userName userEmail userProfilePhoto")
      .lean();
...
exports.loginUser = async (req, res) => {
  ...
  try {
    ...
    const cacheKey = `loginUser:${userEmail}`;
    ...
    try {
      ...
      if (cachedUser) {
        console.log("CACHE HIT ✅ - loginUser");
        existingUser = JSON.parse(cachedUser);
      } else {
        console.log("CACHE MISS ❌ - loginUser");

        existingUser = await User.findOne({ userEmail }).lean();

        if (existingUser) {
          // Store minimal safe data (including password hash for verification)
